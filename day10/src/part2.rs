...
fn recurse(
    ...
    pos: &IVec2,
    n: i32,
    destinations: &mut HashMap<(IVec2, IVec2), usize>,
) {
    if n == 9 {
        if let Some(x) = destinations.get(&(*start, *pos)) {
            destinations.insert((*start, *pos), x + 1);
        } else {
            destinations.insert((*start, *pos), 1);
        }
        return;
    }
...
pub fn solve(s: &str) -> usize {
    ...
    destinations.iter().map(|x| x.1).sum()
}
...
01329801
10456732";
        assert_eq!(solve(&data), 81);
    }

    #[test]
    fn test2() {
        let data = std::fs::read_to_string("input/input.txt").unwrap();
        assert_eq!(solve(&data), 1252);
    }
}
